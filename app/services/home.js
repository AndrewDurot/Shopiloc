...
exports.get_country = async (req, res) => {
    ...
    country_List = removeDuplicates(country_List);
...
exports.get_Search = async (req, res) => {
    try {
        ...
        console.log(code);
        const store = await storeModel.find({postal_code: code, country: req.body.country_list, status: "true"});
        let isExist = false;
        if (store.length > 0) {
            isExist = true;
        }
...
exports.Check_Url = async (req, res) => {
    const store_url = await storeModel.find({"store_url": req.query.url});
...
exports.get_all_state = async (req, res) => {
    const state = [
        {
            "id": "1",
